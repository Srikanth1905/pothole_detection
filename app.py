...
def generate_ghmc_report(detection_data, images, location_info):
    ...
    report_details = [
        ...
        ["Total Potholes:", str(total_detections)],
        ["Average Severity:", f"{detection_data['avg_severity']:.1f}"],
        ["Critical Severity:", f"{severity_distribution['Critical']} ({severity_percentages['Critical']:.1f}%)"],
        ["High Severity:", f"{severity_distribution['High']} ({severity_percentages['High']:.1f}%)"],
        ["Medium Severity:", f"{severity_distribution['Medium']} ({severity_percentages['Medium']:.1f}%)"],
        ["Low Severity:", f"{severity_distribution['Low']} ({severity_percentages['Low']:.1f}%)"]
    ]
...
def process_video(video_file, model, progress_bar):
    ...
    try:
        ...
        # Initialize video writer with H.264 codec and better quality settings
...
with tab3:
    ...
    if map_data:
        ...
        with col2:
            ...
            # Prepare heatmap data with enhanced intensity and better visibility
            heat_data = [[item["lat"], item["lon"], item["severity"] * item["num_detections"] * 5] for item in map_data]
            
            # Add heatmap layer with improved configuration
            HeatMap(
                data=heat_data,
                radius=25,
                blur=20,
